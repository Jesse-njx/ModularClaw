...
python run_cli.py
```
再打开 CLI 输出里显示的状态页面（通常是 `http://localhost:8080`）。CLI 本身不会完整展示后端状态，所以建议通过网页查看 context/status/logs。
...
重要命名规则：`register_module("my_module", ...)` 里的名称必须和模块文件名（`my_module.py`）完全一致。
