...
- **`self.awaiting_user_input`** — `user_input` 工具后为真；**CLI** 据此将 **`Ready to send`** 保持为 **`"pending"`**，直到用户再输入。
...
- 定义内部函数 **`convert(obj)`**：
  ...
  - 标量 → 原样返回。
- 返回字典字段包括：**`id`**、**`version`**、**`context`**、**`status_list`**（经 **`convert`**）、**`logs`**、**`claimed_regions`**、**`loop_count`**、**`tick_count`**、**`awaiting_user_input`**。
